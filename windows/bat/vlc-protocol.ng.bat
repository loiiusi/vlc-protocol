...
set "isweblink="
cmd /c exit /b 0
echo %input%|findstr /i "vlc://weblink?url=" >nul && set isweblink=1
if defined isweblink (
    echo 是 weblink 格式
    :: 提取 url 参数
    set url=!input:vlc://weblink?url==%!
    echo 提取的URL: !url!
    :: URL解码
    call :urldecode url
    echo URL解码后: !url!
...
:: 启动 VLC
echo 正在启动 VLC...
echo 命令行: "%~dp0vlc.exe" --open "!url!"
start "" "%~dp0vlc.exe" --open "!url!"

:: 显示完成信息并暂停
echo.
echo 处理完成。如果 VLC 没有启动，请检查上面的输出信息。
pause
exit /b
